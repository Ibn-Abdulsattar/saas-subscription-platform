...
import {
  fetchProjects, createProject, deleteProject, updateProject, setCurrentProject,
} from "../redux/slices/projectSlice";
import {
  Dialog, DialogTitle, DialogContent, DialogActions,
  TextField, Chip, CircularProgress, Menu, MenuItem, Avatar,
} from "@mui/material";
import {
  Add, MoreVert, Delete, Edit, Visibility,
  PictureAsPdf, FolderOpen, GroupAdd,
...
/* ── Project Card ── */
const ProjectCard = ({ project, role, onView, onEdit, onDelete, onAssignTeam }) => {
    ...
    >
      ...
      <div style={{ padding: 20, flex: 1 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div style={{ flex: 1, cursor: "pointer" }} onClick={() => onView(project)}>
            ...
            <p style={{
              margin: 0, fontSize: "0.83rem", color: "#64748b", lineHeight: 1.5,
          ...
          {(admin || manage) && (
            <button onClick={(e) => setAnchorEl(e.currentTarget)} style={{
      ...
      <div style={{
        ...
        <button onClick={() => onView(project)} style={{
          ...
          borderRadius: 6, border: "none", background: "#eff6ff",
          color: "#3b82f6", fontSize: "0.8rem", fontWeight: 500, cursor: "pointer",
        }}>
      ...
      <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={() => setAnchorEl(null)}>
        <MenuItem onClick={() => { onView(project); setAnchorEl(null); }}>
        ...
        {(admin || manage) && (
          <MenuItem onClick={() => { onAssignTeam(project); setAnchorEl(null); }}>
            <GroupAdd sx={{ fontSize: 16, mr: 1 }} /> Assign Team
          </MenuItem>
        )}
        {admin && (
          <MenuItem onClick={() => { onDelete(project.id); setAnchorEl(null); }} sx={{ color: "#dc2626" }}>
...
export default function ProjectPage() {
  ...
  const [newProject, setNewProject] = useState({ title: "", description: "", pdfFile: null });
  ...
  const handleAssignTeam = async () => {
    ...
    try {
      await axios.post(`${API_URL}/project/${selectedProject.id}/assign-team`, { teamId: selectedTeamId }, { withCredentials: true });
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", padding: "28px 32px", maxWidth: 1200, margin: "0 auto" }}>
      {/* Header */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
        <div>
          <h1 style={{ margin: 0, fontSize: "1.5rem", fontWeight: 700, color: "#0f172a" }}>Projects</h1>
      ...
      {admin && (
        <Dialog open={createOpen} onClose={() => setCreateOpen(false)} maxWidth="sm" fullWidth>
          ...
          <DialogContent sx={{ pt: "12px !important", display: "flex", flexDirection: "column", gap: 2 }}>
            <TextField label="Project Title" fullWidth size="small" required
              value={newProject.title} onChange={(e) => setNewProject({ ...newProject, title: e.target.value })} />
            <TextField label="Description" fullWidth size="small" multiline rows={3}
              value={newProject.description} onChange={(e) => setNewProject({ ...newProject, description: e.target.value })} />
            <div>
              <p style={{ margin: "0 0 6px", fontSize: "0.85rem", color: "#64748b" }}>Project Document (PDF)</p>
              <label style={{
                display: "flex", alignItems: "center", gap: 8, padding: "9px 14px",
                border: "1px dashed #cbd5e1", borderRadius: 8, cursor: "pointer",
          ...
          <DialogActions sx={{ px: 3, pb: 2, gap: 1 }}>
            ...
            <button onClick={handleCreate} disabled={!newProject.title.trim()} style={{
      ...
      {manage && (
        <Dialog open={assignOpen} onClose={() => setAssignOpen(false)} maxWidth="xs" fullWidth>
          <DialogTitle sx={{ fontWeight: 700 }}>Assign Team — {selectedProject?.title}</DialogTitle>
          <DialogContent sx={{ pt: "12px !important" }}>
            {teams.length === 0 ? (
              <p style={{ color: "#64748b", textAlign: "center", padding: "16px 0" }}>No teams available</p>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {teams.map((team) => (
                  <div key={team.id} onClick={() => setSelectedTeamId(team.id)} style={{
                    ...
                    <div>
                      ...
                      {team.description && <div style={{ fontSize: "0.75rem", color: "#64748b" }}>{team.description}</div>}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <DialogActions sx={{ px: 3, pb: 2, gap: 1 }}>
            <button onClick={() => setAssignOpen(false)} style={{
              padding: "8px 18px", borderRadius: 7, border: "1px solid #e2e8f0",
              background: "#fff", cursor: "pointer", fontWeight: 500,
            }}>Cancel</button>
            <button onClick={handleAssignTeam} disabled={!selectedTeamId} style={{
              padding: "8px 18px", borderRadius: 7, border: "none",
